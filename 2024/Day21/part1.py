...
def cost1(a, b):
    ...
    if a in ("1", "4", "7") and b in ("0", "A"):
        ...
        return sum(cost2(c, d) for c, d in pairwise("A" + seq + "A"))
    ...
    if dx == 0 or dy == 0:
        return sum(cost2(c, d) for c, d in pairwise("A" + seq1 + "A"))
    seq2 = ud * abs(dx) + lr * abs(dy)
    return min(
        sum(cost2(c, d) for c, d in pairwise("A" + seq1 + "A")),
        sum(cost2(c, d) for c, d in pairwise("A" + seq2 + "A")),
    )
...
def cost2(a, b):
    ...
    if a == "<" and b in ("^", "A"):
        seq = ">" * dirpad[b][1] + "^"
        return sum(cost3(c, d) for c, d in pairwise("A" + seq + "A"))
    if a in ("^", "A") and b == "<":
        seq = "v" + "<" * dirpad[a][1]
        return sum(cost3(c, d) for c, d in pairwise("A" + seq + "A"))
    ...
    seq1 = lr * abs(dy) + ud * abs(dx)
    if dx == 0 or dy == 0:
        return sum(cost3(c, d) for c, d in pairwise("A" + seq1 + "A"))
    seq2 = ud * abs(dx) + lr * abs(dy)
    return min(
        sum(cost3(c, d) for c, d in pairwise("A" + seq1 + "A")),
        sum(cost3(c, d) for c, d in pairwise("A" + seq2 + "A")),
    )
